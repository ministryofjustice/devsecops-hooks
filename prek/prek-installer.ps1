...
param (
    ...
    [string]$ArtifactDownloadUrl = 'https://github.com/j178/prek/releases/download/v0.2.21',
...
$app_name = 'prek'
$app_version = '0.2.21'
if ($env:PREK_INSTALLER_GHE_BASE_URL) {
  $installer_base_url = $env:PREK_INSTALLER_GHE_BASE_URL
} elseif ($env:PREK_INSTALLER_GITHUB_BASE_URL) {
  $installer_base_url = $env:PREK_INSTALLER_GITHUB_BASE_URL
} else {
  $installer_base_url = "https://github.com"
}
if ($env:PREK_DOWNLOAD_URL) {
  $ArtifactDownloadUrl = $env:PREK_DOWNLOAD_URL
} elseif ($env:INSTALLER_DOWNLOAD_URL) {
  $ArtifactDownloadUrl = $env:INSTALLER_DOWNLOAD_URL
} else {
  $ArtifactDownloadUrl = "$installer_base_url/j178/prek/releases/download/v0.2.21"
}
$auth_token = $env:PREK_GITHUB_TOKEN
...
if ($env:XDG_CONFIG_HOME) {
  $receipt_home = "${env:XDG_CONFIG_HOME}\prek"
} else {
  $receipt_home = "${env:LOCALAPPDATA}\prek"
}

if ($env:PREK_DISABLE_UPDATE) {
...
if ($NoModifyPath) {
    Write-Information "-NoModifyPath has been deprecated; please set PREK_NO_MODIFY_PATH=1 in the environment"
}

if ($env:PREK_NO_MODIFY_PATH) {
    $NoModifyPath = $true
}

$unmanaged_install = $env:PREK_UNMANAGED_INSTALL
...
function Install-Binary($install_args) {
  ...
  $platforms = @{
    "aarch64-pc-windows-gnu" = @{
      "artifact_name" = "prek-aarch64-pc-windows-msvc.zip"
      "bins" = @("prek.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
    ...
    "i686-pc-windows-gnu" = @{
      "artifact_name" = "prek-i686-pc-windows-msvc.zip"
      "bins" = @("prek.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
    ...
    "x86_64-pc-windows-gnu" = @{
      "artifact_name" = "prek-x86_64-pc-windows-msvc.zip"
      "bins" = @("prek.exe")
      "libs" = @()
      "staticlibs" = @()
      "zip_ext" = ".zip"
...
function Invoke-Installer($artifacts, $platforms) {
  ...
  if (($env:PREK_INSTALL_DIR)) {
    $force_install_dir = $env:PREK_INSTALL_DIR
    $install_layout = "flat"
  ...
  if ($install_updater) {
    ...
    [IO.File]::WriteAllLines("$receipt_home/prek-receipt.json", "$receipt", $Utf8NoBomEncoding)
  }
