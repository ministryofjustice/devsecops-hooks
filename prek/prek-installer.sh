...
if [ -n "${PREK_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$PREK_INSTALLER_GHE_BASE_URL"
else
    INSTALLER_BASE_URL="${PREK_INSTALLER_GITHUB_BASE_URL:-https://github.com}"
fi
if [ -n "${PREK_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$PREK_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$INSTALLER_DOWNLOAD_URL"
else
    ARTIFACT_DOWNLOAD_URL="${INSTALLER_BASE_URL}/j178/prek/releases/download/v0.2.21"
fi
if [ -n "${PREK_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$PREK_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${PREK_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$PREK_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${PREK_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$PREK_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${PREK_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${PREK_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${PREK_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.30.0"},"source":{"app_name":"prek","name":"prek","owner":"j178","release_type":"github"},"version":"0.2.21"}
EORECEIPT
...
RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/prek"
...
The installer for prek 0.2.21

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/j178/prek/releases/download/v0.2.21
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set PREK_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        ...
        "prek-aarch64-unknown-linux-musl.tar.gz")
            ...
            _updater_bin=""
            ;;
        "prek-arm-unknown-linux-musleabihf.tar.gz")
            ...
            _updater_bin=""
            ;;
        "prek-armv7-unknown-linux-gnueabihf.tar.gz")
            ...
            _updater_bin=""
            ;;
        "prek-armv7-unknown-linux-musleabihf.tar.gz")
        ...
        "prek-i686-pc-windows-msvc.zip")
            ...
            _updater_bin=""
            ;;
        "prek-i686-unknown-linux-gnu.tar.gz")
            ...
            _updater_bin=""
            ;;
        "prek-i686-unknown-linux-musl.tar.gz")
            ...
            _updater_bin=""
            ;;
        "prek-powerpc64-unknown-linux-gnu.tar.gz")
            ...
            _updater_bin=""
            ;;
        "prek-powerpc64le-unknown-linux-gnu.tar.gz")
        ...
        "prek-riscv64gc-unknown-linux-gnu.tar.gz")
            ...
            _updater_bin=""
            ;;
        "prek-s390x-unknown-linux-gnu.tar.gz")
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="prek-aarch64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="prek-x86_64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-pc-windows-gnu")
            _archive="prek-aarch64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-pc-windows-msvc")
            ...
            _archive="prek-x86_64-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="prek-i686-pc-windows-msvc.zip"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="prek-aarch64-unknown-linux-gnu.tar.gz"
            ...
            _archive="prek-aarch64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "aarch64-unknown-linux-musl-static")
            ...
            ;;
        "arm-unknown-linux-gnueabihf")
            _archive="prek-arm-unknown-linux-musleabihf.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "arm-unknown-linux-musl-staticeabihf")
            ...
            ;;
        "armv7-unknown-linux-gnueabihf")
            _archive="prek-armv7-unknown-linux-gnueabihf.tar.gz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="prek-armv7-unknown-linux-musleabihf.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "i686-pc-windows-msvc")
            ...
            ;;
        "i686-unknown-linux-gnu")
            _archive="prek-i686-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="prek-i686-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "i686-unknown-linux-musl-static")
            ...
            ;;
        "powerpc64-unknown-linux-gnu")
            _archive="prek-powerpc64-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            ;;
        "powerpc64le-unknown-linux-gnu")
            _archive="prek-powerpc64le-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            ;;
        "riscv64gc-unknown-linux-gnu")
            _archive="prek-riscv64gc-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            ;;
        "s390x-unknown-linux-gnu")
            _archive="prek-s390x-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
        ...
        "x86_64-pc-windows-msvc")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="prek-x86_64-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="prek-x86_64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${PREK_INSTALL_DIR:-}" ]; then
        _force_install_dir="$PREK_INSTALL_DIR"
